...
// Deterministic Daily Date & Anime Selector
...
// Local Storage User Record Defaults
...
// Restore Guesses and State on Load / Refresh
...
function updateCareerStats(won) {
  ...
  if (stats.lastPlayedDate === todayKey) return; // Prevent double-scoring on refresh

  stats.played += 1;
  stats.lastPlayedDate = todayKey;

  if (won) {
    // 5 pts for guess #1, 4 for guess #2, etc. (Minimum 1 pt)
...
// Initialize
renderRandomQuote();
restoreDailyState();let animeDatabase = (typeof MASTER_ANIME_DATA !== "undefined") ? MASTER_ANIME_DATA : [];
let targetAnime = null;
let guesses = [];
const MAX_GUESSES = 6;
